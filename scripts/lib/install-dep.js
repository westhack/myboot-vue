...
const getLocalVersion = async (pkg, isDev) => {
  let lsInfo
  try {
    lsInfo = JSON.parse(await exec(`yarn list ${pkg} --json --depth=0 ${isDev ? '--dev' : '--prod'}`))
...
const installFuc = async (pkg, version, pPackage = {}, isDev = false) => {
  ...
  if (v) {
    ...
    await exec(`yarn add ${isDev ? '--dev' : ''} ${pkg}@${version}`)
  ...
  // 不符合要求则按照项目要求回滚
  await exec(`yarn add ${isDev ? '--dev' : ''} ${pkg}@${originPkg[key][pkg]}`)
